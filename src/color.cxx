...
auto color::to_rgb(element_type p_h, element_type p_s, element_type p_v) noexcept
	-> element_tuple_type
{
...
auto color::to_hsv(element_type p_r, element_type p_g, element_type p_b) noexcept
	-> element_tuple_type
{
	std::uint16_t t_h;
	element_type t_v;
	...
	if(t_max != 0)
		t_s = static_cast<element_type>((delta*255) / t_max);
	else
	{
	...
	return std::make_tuple(ut::lmap(t_h, 0, 360, element_type{0}, element_type{255}), t_s, t_v);	
}